...
include '../includes/database-connection.php';               // Create PDO object
...
$temp = $_FILES['image']['tmp_name'] ?? ''; // 허용되는 확장자
$destination = '';                         // 최대 파일 크기
...
$sq1        = "SELECT id, name FROM category:";
...
if ($invalid) {
  ...
  try {
    ...
    if ($destination) {
      ...
      $arguments['image_id'] = $pdo->lastInsertId();          // 이미지를 주가하는 SQL
    }
